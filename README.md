# 👋 Hi, I'm Stefi

🎓 Third-year Computer Engineering Student @ UTCN <br>
💻 Software Engineering Intern @ Microsoft | Research Contributor | Product Owner Intern @ Wolfpack Digital <br>
🧠 Passionate about low-level programming, hardware, systems programming, software engineering, and tech-driven problem-solving

## 🧰 Tech Stack

```bash
💻 Languages: Java, C, C++, C#, Python, VHDL, SQL
🌐 Web: HTML, CSS, JavaScript, Django, React
🔌 Hardware: Arduino, Microcontrollers, FPGA
🧰 Tools: Git, GitHub, Linux, PostgreSQL, Unity, Docker
```

## 💡 What I Care About

I enjoy building software that is not only functional, but useful, reliable, and meaningful.

I am especially interested in systems programming, low-level engineering, distributed systems, hardware-software interaction, and products that solve real problems for real people.

## 📫 Connect with Me

```bash
💼 LinkedIn: linkedin.com/in/stefania-mozacu
```

## 📊 GitHub Activity

![Stefi's GitHub stats](https://github-readme-stats.vercel.app/api?username=stefi19&show_icons=true&hide_border=true&hide_rank=true)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=stefi19&layout=compact&hide_border=true)
